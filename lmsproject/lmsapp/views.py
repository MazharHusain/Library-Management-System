...
def contactus(request):
    if request.method=="POST":
        ...
        enq=Enquiry(name=name, gender=gender, address=address, contactno=contactno, enquirytext=enquirytext, enquirydate=enquirydate)
        enq.save()
        sendsms(contactno)
...
def registration(request):
    if request.method=="POST":
        ...
        stu.save()
        log.save()
        subject='Important mail from KMCLU-Lucknow'
